...
const skillSchema = z.object({
  skills: z.array(z.object({
    title:   z.string(),
    insight: z.string(),    // 一句话能力描述，≤60字
    body:    z.string(),    // markdown 能力卡
    tags:    z.array(z.string()),
  })).max(5),
})

const EXTRACT_SYSTEM = `你是一个高标准的 AI Agent 能力工程师，负责从对话中提炼真正可复用的 Skill 能力卡。

Skill 不是文章摘要，也不是文档总结。Skill 的定位是：未来遇到相似任务时，Agent 可以直接调用的一套做事策略 / 检查清单 / 操作规程。

【筛选标准——极为严格，宁缺毋滥】
只提炼满足以下任一条件的内容：
- 形成了未来可复用的操作流程、排查流程、设计决策框架或评审清单
- 解决了有深度的技术问题，并沉淀出可迁移的根因定位方法和修复策略
- 总结出可指导 Agent 下次行动的工具使用策略、上下文策略、检索策略或记忆策略
- 记录了一个能显著避免重复踩坑的工程约束、反例或风险判断
...
- 对话中只是顺带一提、没有展开讨论的点
- 已有 Skill 的重复
- 只是在复述一篇文档、博客或搜索结果，没有沉淀成行动规则
- 只能用于单次任务的项目流水账

【每条 Skill 格式要求】
- title：≤20字，准确概括核心知识点
- insight：一句话能力描述（≤60字），说清楚"下次遇到什么任务时，用它怎么做"
- body：短而硬的 markdown 能力卡，必须使用以下固定结构：
  ## 适用场景
  - 2-4 条触发条件，说明什么时候该调用这个 skill
  ## 操作步骤
  - 3-6 条可执行步骤，用祈使句，不写空泛理念
  ## 检查清单
  - 3-6 条验收点，用来判断这次是否做对
  ## 反例
  - 2-4 条不要这么做的情况
  ## 证据
  - 1-3 条来自对话的具体依据，保留日期或关键词
  * 长度：250-800 字，宁可短，不写背景科普
  * 语言：中文，技术术语保持英文
