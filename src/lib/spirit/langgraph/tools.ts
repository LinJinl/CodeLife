...
规则：
- 只加明确需要的域，不确定则不加
- 用户明确说"联网/搜索/深度检索/研究/调研/最新/前沿/现在业界"时必须加 web
...
export async function inferDomainsWithAI(
  ...
  try {
    ...
    const domains = new Set<ToolDomain>([
      ...(result.domains as ToolDomain[]),
      ...inferExtraDomains(userMessage),
    ])
...
export function inferExtraDomains(userMessage: string): ToolDomain[] {
  ...
  if (/搜索|搜一下|搜集|查一下|查查|找.*资料|相关资料|最新|网上|在线|网页|链接|url|http|google|bing|了解一下|学习资料|联网|深度检索|research_web|研究|调研|前沿|业界|行业.*讨论|现在.*讨论|最先进|state of the art|sota/.test(text)) {
    extra.push('web')
  }
