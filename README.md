...
| 偏好画像 | 从对话中提炼的行为习惯（置信度加权） | 对话中持续更新 |
...
| Agent | 职责 | 工具集 |
|-------|------|--------|
| 青霄（qingxiao） | 全能主理，直通模式下独立完成全部任务 | 按请求域动态加载 |
| 搜寻使（search_agent） | 专注联网信息检索 | `web_search`、`fetch_url` |
| 算法师（code_agent） | 读取分析刷题与博客数据 | 博客、刷题、藏经阁工具 |
| 星盘官（planner_agent） | 分析修炼规律、制定计划 | 日志、刷题、偏好、对话历史工具 |
...
spirit: {
  enabled:             true,
  name:                '青霄',         // 器灵名字，随意改
  model:               'gpt-4o-mini', // 支持任何 OpenAI 兼容模型（对话用）
  reflectModel:        'gpt-4o',      // 可选，周期记忆生成与偏好提炼用（不填则复用 model）
...
**周期记忆生成**（由 `POST /api/spirit/sync` 触发）：

| 任务 | 触发条件 | 产物 |
|------|----------|------|
| 周规律分析 | 每周一 | `content/spirit/patterns/{week}.json` |
| 人格档案更新 | 每 7 天 | `content/spirit/persona.json` |
| 技能卡提炼 | 有未处理对话时 | `content/spirit/skills/` |
| 偏好画像提炼 | 有未处理对话时 | `content/spirit/preferences.json` |

技能卡和偏好画像也可在管理页单独手动触发（`POST /api/spirit/skills`、`POST /api/spirit/preferences`）。
...
└── src/
    ...
    │   │   ├── context/      页面上下文注入
    │   │   ├── vows/         誓约 CRUD
    │   │   ├── skills/       技能卡 CRUD + 手动触发提炼
    │   │   ├── preferences/  偏好画像 CRUD + 手动触发提炼
    │   │   └── sync/         数据同步
    │   ├── blog/             博客页面
    │   ├── github/           GitHub 声望页面
    │   ├── leetcode/         铸剑台页面
    ...
    ├── components/
    │   ├── spirit/
    │   │   ├── SpiritWidget.tsx   器灵对话容器（布局 + 拖拽）
    │   │   ├── MessageItem.tsx    单条消息渲染（memo 优化）
    │   │   ├── useSpiritChat.ts   对话状态 Hook（SSE + 权限确认）
    │   │   └── types.ts           共享类型 + 斜杠命令常量
    ...
    └── lib/
        ...
        └── spirit/           器灵 AI 核心
            ...
            │   ├── agent-config.ts  Agent 元数据单一来源
            │   ├── tools.ts         工具域过滤与按需加载
            │   └── nodes/           planner / supervisor / executor / synthesizer
            ├── tools/        内置工具（29 个，按域分组）
            ├── memory.ts     五层记忆读写
            ├── sync.ts       数据同步与周期记忆生成
            ├── prompt.ts     System Prompt 构建（Tier1 注入 + Tier2 按需）
            ├── hybrid-search.ts  BM25 + Embedding 混合检索
            ├── skill-extractor.ts  技能卡 LLM 提炼
            └── preference-extractor.ts  偏好画像 LLM 提炼
```
